    ...
    <div className="flex flex-col md:flex-row min-h-full bg-slate-50 relative overflow-x-hidden">
      ...
      {showErrorModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-red-50">
              <div>
                <h2 className="text-xl font-black text-red-600 tracking-tighter uppercase">Diagnóstico Técnico</h2>
                <p className="text-xs font-bold text-red-500/70 tracking-widest uppercase">Últimos {lastErrors.length} errores detectados</p>
              </div>
              <button onClick={() => setShowErrorModal(false)} className="p-2 bg-white text-slate-400 rounded-xl shadow-sm">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50">
              {lastErrors.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="font-black text-slate-400 uppercase tracking-tighter">No se han registrado errores críticos</p>
                </div>
              ) : (
                lastErrors.map((err, idx) => (
                  <div key={idx} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[10px] font-black uppercase tracking-widest bg-red-100 text-red-600 px-2 py-0.5 rounded-md">Tabla: {err.table}</span>
                      <span className="text-[10px] text-slate-400 font-mono">{new Date(err.timestamp).toLocaleTimeString()}</span>
                    </div>
                    <div className="text-xs font-mono bg-slate-900 text-emerald-400 p-3 rounded-xl overflow-x-auto whitespace-pre-wrap leading-relaxed border border-slate-800">
            ...
            <div className="p-4 bg-white border-t border-slate-100 flex gap-3">
              <button
                onClick={() => { setLastErrors([]); setShowErrorModal(false); }}
                className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl font-black uppercase tracking-tighter text-sm active:scale-95 transition-all"
              >
                Limpiar Historial
              </button>
              <button
                onClick={() => { processQueue(true); setShowErrorModal(false); }}
                className="flex-1 py-4 bg-emerald-600 text-white rounded-2xl font-black uppercase tracking-tighter text-sm shadow-lg shadow-emerald-200 active:scale-95 transition-all"
              >
                Reintentar Ahora
              </button>
            </div>
      ...
      {showErrorModal && (
        <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-[95%] max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col max-h-[85vh] border border-white/20">
            <div className="p-7 border-b border-slate-100 flex items-center justify-between bg-red-50/50">
              <div>
                <h2 className="text-xl font-black text-red-600 tracking-tighter uppercase leading-none">Diagnóstico Taller</h2>
                <p className="text-[9px] font-black text-red-500/60 tracking-[0.2em] uppercase mt-1">Identificador de Errores APK</p>
              </div>
              <button onClick={() => setShowErrorModal(false)} className="w-10 h-10 bg-white text-slate-400 rounded-xl shadow-sm flex items-center justify-center active:scale-90 transition-all">
                <i className="fa-solid fa-xmark text-lg"></i>
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-slate-50/50">
              {lastErrors.length === 0 ? (
                <div className="text-center py-20 px-10">
                  ...
                  <h3 className="font-black text-slate-800 uppercase tracking-tighter text-lg">Todo en Orden</h3>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-2">No hay errores críticos registrados en esta sesión.</p>
                </div>
              ) : (
                lastErrors.map((err, idx) => (
                  <div key={idx} className="bg-white p-5 rounded-[2rem] border border-slate-200 shadow-sm relative overflow-hidden group">
                    <div className="absolute top-0 left-0 w-1.5 h-full bg-red-500"></div>
                    <div className="flex items-center justify-between mb-3 px-1">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black uppercase tracking-widest bg-red-600 text-white px-3 py-1 rounded-full shadow-lg shadow-red-200">
                          {err.table}
                        </span>
                      </div>
                    ...
                    <div className="text-[11px] font-mono bg-slate-900 text-emerald-400 p-4 rounded-2xl overflow-x-auto whitespace-pre-wrap leading-relaxed border border-white/5 shadow-inner">
            ...
            <div className="p-6 bg-white border-t border-slate-100 flex gap-4">
              <button
                onClick={() => { setLastErrors([]); setShowErrorModal(false); }}
                className="flex-1 py-5 bg-slate-100 text-slate-500 rounded-3xl font-black uppercase tracking-widest text-[10px] active:scale-95 transition-all flex items-center justify-center gap-2"
              >
                <i className="fa-solid fa-trash-can opacity-60"></i>
                Limpiar Historial
              </button>
              <button
                onClick={() => { processQueue(true); setShowErrorModal(false); }}
                className="flex-[1.5] py-5 bg-emerald-600 text-white rounded-3xl font-black uppercase tracking-widest text-[10px] shadow-2xl shadow-emerald-600/30 active:scale-95 transition-all flex items-center justify-center gap-2"
              >
