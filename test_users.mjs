...
async function check() {
    const { data: users } = await supabase.from('users').select('id, username, managed_by').limit(5);
    console.log("Users:", users);

    const { data: simulated } = await supabase.from('simulated_orders').select('*').limit(1);
    console.log("Simulated order:", simulated);
}
