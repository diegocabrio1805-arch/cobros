...
async function main() {
  ...
  const logId = 'LOG-MIG-L-be1ff7a0-3c60-4cbd-87b7-6e795390bcd6';
  const loanId = 'L-be1ff7a0-3c60-4cbd-87b7-6e795390bcd6';
  const newAbonoAmount = 750000;
  ...
  const newInstallments = oldInstallments.map(inst => ({
    ...inst,
    paidAmount: 0,
    status: 'Pendiente' // Using 'Pendiente' as defined in PaymentStatus.PENDING
  }));
  ...
  console.log(`- installments: 1 to 5 fully paid count: ${newInstallments.filter(i => i.status === 'Pagado').length}`);
