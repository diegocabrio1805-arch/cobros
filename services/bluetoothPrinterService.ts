...
const KEEPER_INTERVAL_MS = 5000; // Reducido para mayor respuesta en segundo plano
...
export const printText = async (rawText: string, retryCount = 0): Promise<boolean> => {
    if (isCurrentlyPrinting && retryCount === 0) {
        console.warn("[Bluetooth] Impresora ocupada. Encolando ticket para imprimir secuencialmente.");
        queuePrintJob(rawText);
        return true; // Retornamos true porque el trabajo está asegurado en la cola
    }
    ...
    try {
        ...
        console.warn("[Bluetooth] Print failed. Attempting recovery...", e);
        if (retryCount < 1) { // Solo 1 reintento de reconexión forzada
        ...
        // Si llegamos aquí es que falló el primer intento y el reintento de recuperación
        if (retryCount === 0) {
            console.error("[Bluetooth] Fallo crítico de impresión tras recuperación. Encolando ticket.");
            queuePrintJob(rawText);
        }
        return false;
    } finally {
