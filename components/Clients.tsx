...
const Clients: React.FC<ClientsProps> = ({ state, addClient, addLoan, updateClient, updateLoan, deleteCollectionLog, updateCollectionLog, updateCollectionLogNotes, addCollectionAttempt, globalState, onForceSync, deleteLoan, recalculateLoanStatus, setActiveTab, fetchClientPhotos, deleteClient, addBulkData, undoLastBulkImport, renewLoan, activeLocation, initialDossierClientId, onClearInitialDossier, setState, pushLoan }) => {
  ...
  const handleDossierAction = async (type: CollectionLogType, customAmount?: number) => {
    ...
    try {
      ...
      // VALIDACIÓN: El abono no puede ser mayor al saldo calculado desde logs (igual que muestra la pantalla)
                                  ...
                                  <tbody className="divide-y divide-slate-100 text-[10px] font-bold">
                                    {(Array.isArray(state.loans) ? state.loans : []).filter(l => l.clientId === clientInLegajo.id).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(loan => {
                                      ...
                                      if (loan.status === 'Pagado' || loan.status === 'PAID') {
                                        ...
                                        // Crédito ACTIVO o en MORA: usar el cálculo oficial unificado
                                        atraso = getDaysOverdue(loan, state.settings);
                                      }
