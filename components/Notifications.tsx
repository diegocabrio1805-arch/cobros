...
const Notifications: React.FC<NotificationsProps> = ({ state }) => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn max-w-4xl mx-auto pb-20">
      <div className="flex justify-between items-center bg-white p-6 rounded-none border border-slate-100 shadow-sm">
        <div>
        ...
        <div className="bg-blue-50 px-4 py-2 rounded-none border border-blue-100">
      ...
      <div className="grid grid-cols-1 gap-4">
        {alerts.length === 0 ? (
          <div className="bg-white py-20 rounded-none border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-slate-400">
            <i className="fa-solid fa-circle-check text-5xl mb-4 text-green-200"></i>
            <p className="text-lg font-bold">{t.notifications.clean}</p>
          </div>
        ) : (
          alerts.map((alert) => (
            <div key={alert.id} className={`bg-white p-6 rounded-none border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 hover:shadow-md transition-all border-l-4 ${alert.isOverdue ? 'border-l-red-500' : 'border-l-amber-500'}`}>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-2">
                  <span className={`px-2 py-0.5 rounded-none text-[8px] font-black uppercase ${alert.isOverdue ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                <button
                  ...
                  className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white px-6 py-4 rounded-none font-black text-[10px] uppercase tracking-widest shadow-lg shadow-red-600/20 transition-all active:scale-95"
                >
