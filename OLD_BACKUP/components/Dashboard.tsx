...
const Dashboard: React.FC<DashboardProps> = ({ state }) => {
  ...
  const scopedLoans = useMemo(() => {
    if (state.currentUser?.role === Role.MANAGER) {
      ...
      return state.loans.filter(l => l.collectorId === managerId || (l.collectorId && myCollectorIds.includes(l.collectorId)));
    }
    if (state.currentUser?.role === Role.ADMIN) {
      return state.loans.filter(l => {
        const collector = state.users.find(u => u.id === l.collectorId);
    ...
    return state.loans;
  }, [state.loans, state.users, state.currentUser]);

  const scopedPayments = useMemo(() => {
    const loanIds = new Set(scopedLoans.map(l => l.id));
    return state.payments.filter(p => loanIds.has(p.loanId));
  }, [state.payments, scopedLoans]);

  const totalProfit = scopedLoans.reduce((acc, l) => acc + (l.totalAmount - l.principal), 0);
  ...
  return (
    <div className="space-y-4 animate-fadeIn pb-24 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-[1.5rem] border border-slate-100 shadow-sm">
        <div>
          <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter leading-none">Resumen Operativo</h2>
