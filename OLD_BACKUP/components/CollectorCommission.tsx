...
const CollectorCommission: React.FC<CollectorCommissionProps> = ({ state, setCommissionPercentage, updateCommissionBrackets, deleteCollectionLog }) => {
  ...
  const calculateStatsForCollector = (targetUserId: string | 'all') => {
    ...
    const logsHoy = state.collectionLogs.filter(log => {
      const matchesUser = targetUserId === 'all' ? true : (log.recordedBy === targetUserId);
      const logDate = new Date(log.date);
      // Comparación estricta de fecha local para asegurar coincidencia con "HOY"
      const isSameDate = logDate.getDate() === todayDate.getDate() &&
        logDate.getMonth() === todayDate.getMonth() &&
        logDate.getFullYear() === todayDate.getFullYear();
      return matchesUser && isSameDate && !log.isOpening;
    });

    // Recaudo Hoy incluye Efectivo y Transferencias, excluyendo Renovaciones si se desea "Dinero Real"
    // Sin embargo, para "Incentivo", usualmente todo PAGO cuenta.
  ...
  const excelLogs = useMemo(() => {
    if (!state.collectionLogs || state.collectionLogs.length === 0) return [];
    ...
    return state.collectionLogs.filter(log => {
      // 1. Siempre excluir aperturas de caja
      if (log.isOpening) return false;

      // 2. Filtro de Fechas (Por TimeStamp es más seguro)
      const logTime = new Date(log.date).getTime();
      if (logTime < startMs || logTime > endMs) return false;

      // 3. Filtro de Usuario (Ruta/Gestor)
      // Si selecciono 'all', pasa CUALQUIER log (siempre que esté en el rango de fechas)
      // Si selecciono un usuario, el log debe pertenecer a él.
      if (selectedHistoricalRoute !== 'all') {
        if (log.recordedBy !== selectedHistoricalRoute) return false;
      }

      // 4. Filtro por Tipo de Pago
      if (paymentTypeFilter === 'nopay') return log.type === CollectionLogType.NO_PAGO;

      const isVirtual = !!log.isVirtual; // Fuerza booleano, undefined -> false
      const isRenewal = !!log.isRenewal; // Fuerza booleano, undefined -> false

      if (paymentTypeFilter === 'virtual') return isVirtual;
      if (paymentTypeFilter === 'renewal') return isRenewal;

      if (paymentTypeFilter === 'cash') {
        // Cash es Pago que NO es Virtual Y NO es Renovación
        return log.type === CollectionLogType.PAYMENT && !isVirtual && !isRenewal;
      }

      // Si paymentTypeFilter es 'all', pasan todos (efectivo, virtual, renovacion, no pago)
      return true;
    }).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [state.collectionLogs, excelStartDate, excelEndDate, selectedHistoricalRoute, paymentTypeFilter]);
  ...
  return (
    <div className="flex flex-col space-y-4 animate-fadeIn max-w-7xl mx-auto pb-24 px-1 md:px-0">
      ...
      {showBracketModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-[300] p-4">
      ...
      {showExcelModal && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex items-center justify-center z-[250] p-0 md:p-4 overflow-hidden animate-fadeIn">
                      ...
                      <tr key={log.id} className="hover:bg-slate-50 transition-colors text-[11px] font-bold">
                        ...
                        <td className="px-5 py-3 no-print">
                          <div className="flex justify-center gap-1">
                            ...
                            <button onClick={() => { if (confirm('¿BORRAR ESTE PAGO DEFINITIVAMENTE? SE REVERTIRÁN LOS SALDOS.')) deleteCollectionLog?.(log.id); }} className={`w-8 h-8 rounded-lg bg-red-50 text-red-600 flex items-center justify-center shadow-sm ${!isPowerUser ? 'hidden' : ''}`}><i className="fa-solid fa-trash"></i></button>
                          </div>
                        </td>
                      </tr>
                    );
      ...
      {showGlobalSummary && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md flex items-center justify-center z-[200] p-4 overflow-hidden">
