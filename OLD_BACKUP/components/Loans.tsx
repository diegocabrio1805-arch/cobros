...
import { AppState, Loan, LoanStatus, Role, PaymentStatus, CollectionLog, CollectionLogType } from '../types';
import { formatCurrency, generateReceiptText, getDaysOverdue, formatDate } from '../utils/helpers';
...
const Loans: React.FC<LoansProps> = ({ state, addCollectionAttempt, deleteCollectionLog }) => {
  const [viewMode, setViewMode] = useState<'gestion' | 'vencidos'>('gestion');
  ...
  const filteredLoans = useMemo(() => {
    return state.loans.filter((loan) => {
      ...
      if (viewMode === 'vencidos') {
        return isAssigned && matchesSearch && getDaysOverdue(loan) > 0;
      }

      // Solo mostrar créditos que no estén pagados en la vista de gestión
      return isAssigned && matchesSearch && loan.status !== LoanStatus.PAID;
    }).sort((a, b) => {
      if (viewMode === 'vencidos') {
      ...
      doc.write(`
            ...
            <head>
                <style>
                    ...
                    /* Ensure exact fit for thermal printers */
                    @page { margin: 0; }
                </style>
            </head>
  ...
  const handlePrintOverdueReport = () => {
    ...
    const tableRows = filteredLoans.map(loan => {
      const client = state.clients.find(c => c.id === loan.clientId);
      const mora = getDaysOverdue(loan);
  ...
  const handleQuickAction = async (loanId: string, type: CollectionLogType, customAmount?: number, isVirtual: boolean = false, isRenewal: boolean = false) => {
    ...
    try {
      ...
      if (client && type === CollectionLogType.PAYMENT) {
        ...
        const expiryDate = loan.installments[loan.installments.length - 1].dueDate;
        const paidInstCount = loan.installments.filter(i => i.status === PaymentStatus.PAID).length + 1;
        const overdueDays = getDaysOverdue(loan);
        ...
        const receiptText = generateReceiptText({
          ...
          remainingBalance: Math.max(0, loan.totalAmount - totalPaidOnLoan),
  ...
  return (
    <div className="space-y-4 md:space-y-6 pb-20 animate-fadeIn px-1">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 md:p-6 rounded-2xl md:rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div className="w-full md:w-auto text-center md:text-left">
          <h2 className="text-xl md:text-2xl font-black text-slate-900 uppercase tracking-tighter flex items-center justify-center md:justify-start gap-3">
            <i className={`fa-solid ${viewMode === 'gestion' ? 'fa-money-bill-wave text-blue-600' : 'fa-triangle-exclamation text-red-600'}`}></i>
            {viewMode === 'gestion' ? t.loans.title : 'Cartera Vencida'}
          </h2>
          <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
            {viewMode === 'gestion' ? t.loans.subtitle : 'Créditos con pagos atrasados ordenados por mora'}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto items-center">
          ...
          <div className="relative w-full md:w-80">
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl md:rounded-2xl py-3 md:py-4 pl-10 md:pl-12 pr-4 text-xs md:text-sm font-black text-slate-800 outline-none focus:ring-2 focus:ring-blue-500 shadow-inner"
            />
            <i className="fa-solid fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 text-xs md:text-sm"></i>
          </div>
        </div>
      </div>
      ...
      {viewMode === 'gestion' ? (
            ...
            filteredLoans.map((loan) => {
              ...
              return (
                <div key={loan.id} className="bg-white rounded-2xl md:rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group overflow-hidden flex flex-col">
                  ...
                  <div className="p-3 md:p-4 bg-slate-50 border-t border-slate-100 grid grid-cols-2 gap-2 md:gap-3">
                    <button
        ...
        /* VISTA DE COBROS VENCIDOS (TIPO EXCEL) */
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-xl overflow-hidden flex flex-col">
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left border-collapse min-w-[900px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredLoans.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-24 text-center text-slate-300 font-black uppercase tracking-[0.3em]">No hay créditos en mora actualmente</td>
                  </tr>
                ) : (
                  filteredLoans.map((loan) => {
                    const client = state.clients.find(c => c.id === loan.clientId);
          ...
          <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest">
            <span>Total registros: {filteredLoans.length}</span>
            <span>Reporte de Morosidad Consolidado</span>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-[150] p-2 overflow-y-auto">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-sm overflow-hidden animate-scaleIn border border-white/20">
            <div className="p-5 md:p-6 bg-slate-900 text-white flex justify-between items-center sticky top-0 z-10">
              <div>
                <h3 className="text-base md:text-lg font-black uppercase tracking-tighter">Registrar Abono</h3>
              </div>
            ...
            <div className="p-5 md:p-6 space-y-4 md:space-y-6">
              <div className="grid grid-cols-3 gap-2">
                ...
                <button onClick={() => setMethod('renewal')} className={`py-2 rounded-lg text-[8px] font-black uppercase border transition-all ${isRenewalPayment ? 'bg-amber-600 text-white shadow-md' : 'bg-slate-50 text-slate-400 active:bg-slate-100'}`}>Liquidar</button>
              </div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-5 py-8 md:py-10 bg-slate-50 border border-slate-200 rounded-2xl md:rounded-[2.5rem] text-3xl md:text-5xl font-black text-center text-slate-900 outline-none focus:ring-4 focus:ring-emerald-500/20 shadow-inner"
                  placeholder="0"
                />
              </div>

              <button
                onClick={() => selectedLoanId && handleQuickAction(selectedLoanId, CollectionLogType.PAYMENT, paymentAmount, isVirtualPayment, isRenewalPayment)}
                disabled={isProcessingPayment}
                className="w-full py-4 md:py-5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl md:rounded-[2rem] font-black uppercase text-xs md:text-sm tracking-widest shadow-2xl active:scale-95 transition-all disabled:opacity-50"
              >
                {isProcessingPayment ? <i className="fa-solid fa-circle-notch animate-spin mr-2"></i> : 'Confirmar Cobro e Imprimir'}
              </button>
            </div>
          </div>
        </div>
      ...
      {receipt && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl flex items-center justify-center z-[160] p-4 overflow-y-auto">
          <div className="bg-white rounded-[2rem] p-6 md:p-10 text-center max-w-sm w-full animate-scaleIn shadow-2xl">
            <div className="w-16 h-16 md:w-20 md:h-20 bg-green-100 text-green-600 rounded-2xl md:rounded-[2.5rem] flex items-center justify-center mx-auto mb-6 text-3xl shadow-xl border border-green-200"><i className="fa-solid fa-check-double"></i></div>
            <h3 className="text-xl font-black text-slate-800 mb-6 uppercase tracking-tighter">¡Gestión Exitosa!</h3>
            <div className="bg-slate-50 p-4 md:p-6 rounded-xl md:rounded-2xl font-mono text-[9px] md:text-[10px] text-left mb-8 max-h-40 overflow-y-auto border border-slate-200 text-black font-black shadow-inner whitespace-pre-wrap">{receipt}</div>
            <button onClick={resetUI} className="w-full py-4 bg-slate-900 text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-2xl active:scale-95 transition-all">Cerrar y Continuar</button>
          </div>
        </div>
      )}
    </div>
