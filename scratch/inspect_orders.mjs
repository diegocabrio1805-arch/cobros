...
async function run() {
    ...
    const { data: orders, error } = await supabase.from('simulated_orders').select('*').order('created_at', { ascending: false }).limit(5);
    if (error) {
        console.error("Query Error:", error);
    } else {
        console.log("Recent orders in db:");
        orders.forEach(o => {
            console.log(`- ID: ${o.id} | Client: ${o.client_name} | CreatedAt: ${o.created_at} | UpdatedAt: ${o.updated_at}`);
        });
    }
}
