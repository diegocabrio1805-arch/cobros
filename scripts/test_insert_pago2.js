...
function insertTestLog() {
    ...
    const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
            fs.writeFileSync('c:\\Users\\HP\\.gemini\\antigravity\\scratch\\cobros\\insert_result.txt', `STATUS: ${res.statusCode}\nRESPONSE: ${data}`);
        });
    });

    req.on('error', (e) => {
        fs.writeFileSync('c:\\Users\\HP\\.gemini\\antigravity\\scratch\\cobros\\insert_result.txt', `PROBLEM: ${e.message}`);
    });
