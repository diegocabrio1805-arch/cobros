...
CREATE POLICY "Gerente ve perfiles de su sucursal" ON public.profiles FOR ALL USING (
    (SELECT role FROM public.profiles WHERE id = auth.uid()) = 'Gerente' AND 
...
CREATE POLICY "Gerente gestiona clientes de sucursal" ON public.clients FOR ALL USING (
   (SELECT role FROM public.profiles WHERE id = auth.uid()) IN ('Administrador', 'Gerente') AND 
   branch_id = (SELECT branch_id FROM public.profiles WHERE id = auth.uid())
);
...
CREATE POLICY "Cobrador gestiona sus pagos" ON public.payments FOR ALL USING (
   collector_id = auth.uid() OR branch_id = (SELECT branch_id FROM public.profiles WHERE id = auth.uid())
);
...
CREATE POLICY "Cobrador gestiona sus propios logs" ON public.collection_logs FOR ALL USING (
  recorded_by = auth.uid() OR branch_id = (SELECT branch_id FROM public.profiles WHERE id = auth.uid())
);
...
-- Nota: Estas políticas permiten a los cobradores VER los datos de lectura de su sucursal para que
-- el modo Offline (pull_data) no falle, pero solo les permite editar lo que el frontend les habilite.
-- Es un blindaje nivel "Tenant" por Sucursal.
